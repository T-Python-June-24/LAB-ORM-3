...
def product_list(request, category_name):
    ...
    if category_name == "all":
        ...
        inventory=inventory.annotate(supplier_count=Count('suppliers'))
    else:
        inventory = Product.objects.filter(categories__name__in=[category_name])
    
    inventory = inventory.annotate(product_count=Count("id"))

    page_number = request.GET.get("page", 1)
    paginator = Paginator(inventory, 3)  
    inventory_page = paginator.get_page(page_number)
    price_product=inventory.aggregate(Sum('price'))


    return render(request, "inventory/product_list.html", {
        "inventory": inventory_page,
        "category_name": category_name,
        "products": inventory_page, 
        "price":price_product['price__sum']
    })
...
def product_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
             form.save()
             messages.success(request, "Products have been successfully added", "alert-success")
             return redirect('inventory/product_list.html')
    else:
        form = ProductForm()
...
def product_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        return redirect('inventory:product_list', category_name='all')
